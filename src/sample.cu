...
__global__ void kernGetCoherentVal(int N, int *particleArrayIndices, PointType *pos, PointType *coherentPos) {
    int index = threadIdx.x + (blockIdx.x * blockDim.x);
    if (index >= N) return;
...
__global__ void kernUniformDownSample(int N, PointType *pts_in, PointType *pts_out, Eigen::Vector4i grid_res,
              Eigen::Vector4i grid_count, int *dev_array_indices, int *dev_grid_indices){
    int index = threadIdx.x + (blockIdx.x * blockDim.x);
    if (index < N){
//        for (int i = 0; i < grid_res[0]; i++){
//            for (int j = 0; j < grid_res[1]; j++){
//                for (int k = 0; k < grid_res[2]; k++){
//                    int idx = i + grid_count[0] * j + grid_count[1] * k;
//                    float min_dist = FLT_MAX;
//                    if (dev_grid_indices[index] == idx){
//                        atoimicMin(input_->points[(*indices_)[cp]].getVector4fMap () - ijk.cast<float> ()).squaredNorm ();)
//                    }
//                }
//            }
//        }
...
UniformDownSample::~UniformDownSample() {
    ...
    cudaFree(dev_coherent_pc);
    dev_min = NULL;
    dev_max = NULL;
    dev_grid_indices = NULL;
    dev_array_indices = NULL;
    dev_coherent_pc = NULL;
}
...
void UniformDownSample::downSample(const pcl::PointCloud<PointType >::ConstPtr input) {
    ...
    thrust::device_ptr<int> dev_thrust_particleGridIndices =  thrust::device_ptr<int>(dev_grid_indices);
    thrust::device_ptr<int> dev_thrust_particleArrayIndices = thrust::device_ptr<int>(dev_array_indices);
    // sort inplace
    ...
    cudaMalloc((void**)&dev_coherent_pc, N * sizeof(PointType));
    checkCUDAError("malloc dev_coherent_pc error");
    kernGetCoherentVal <<<fullBlockPerGrid_points, blockSize >>> (N, dev_array_indices, dev_pc, dev_coherent_pc);
...
//    thrust::copy(dev_thrust_particleGridIndices, dev_thrust_particleGridIndices + num_unique, unique_indices.begin());
//
...
//        checkCUDAErrorWithLine("kernIdentifyCellStartEnd Failed");
    ...
    int * grid_indices = new int[N];
    cudaMemcpy(grid_indices, dev_grid_indices, N  * sizeof(int), cudaMemcpyDeviceToHost);
