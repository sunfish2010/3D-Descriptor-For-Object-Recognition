...
__global__ void kernSearchRadius(int N, int n, const PointType *surface, float radius, const int *feature_indices,
      const Eigen::Vector4f inv_radius, const Eigen::Vector4i min_pi, int * neighbor_indices, int* num_neighbors) {
        ...
        if (isfinite(pt.x) && isfinite(pt.y) && isfinite(pt.z)){
            ...
            for (int idx = 0; i < n; ++i ){
                if (i > search_range[idx] && i < search_range[idx + 3]
                    ...
                    neighbor_indices[idx * n + num_neighbors[idx]] = index;
...
//        checkCUDAError("memcpy gi failed");
        ...
        kernSearchRadius<<<fullBlockPerGrid_points, blockSize>>> (_N_surface, _N_features, dev_pos_surface,
                _radius, dev_features_indices, inv_radius, min_p, dev_neighbor_indices, dev_num_neighbors);
